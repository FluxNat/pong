...
export function getRepoNames() {
    ...
    return ["FluxNat/pong", "ghcr.io/FluxNat/pong"];
}
 ...
 * docker buildx build -f docker/dockerfile --platform linux/amd64 -t FluxNat/pong:upload-artifact --build-arg VERSION --build-arg GITHUB_TOKEN --target upload-artifact . --progress plain
 * @param {string} version Version
...
export async function createReleasePR(version, previousVersion, dryRun, branchName = "release", githubRunId = null) {
    ...
    const artifactLink = githubRunId
        ? `https://github.com/FluxNat/pong/actions/runs/${githubRunId}/workflow`
        : `https://github.com/FluxNat/pong/actions/workflows/beta-release.yml`;
